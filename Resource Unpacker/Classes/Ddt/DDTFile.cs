...
namespace Resource_Unpacker.Classes.Ddt
{
    ...
    public class DdtFile
    {
        ...
        public string DdtInfo
        {
            get
            {
                return BaseWidth.ToString() + "x"+ BaseHeight.ToString()+", "+ Format.ToString() +" ("+ Usage.ToString() + ", " + Alpha.ToString()+")";
            }
        }
        ...
        private BitmapSource GetBitmap()
        {
            ...
            BitmapSource bitmap;
            bitmap = BitmapSource.Create(ddtImage.Width, ddtImage.Height * 2, 96, 96, PixelFormats.Pbgra32, null, image.ToArray(), 4 * ddtImage.Width);
